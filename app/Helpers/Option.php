...
class Option {
	 ...
	 * @return false|string
	 */
	public static function getUserAccessToken( int $userID = 0 ) {
		$userID = self::safeGetUserId( $userID );
		if ( empty( $userID ) ) {
			return false;
		}

		return (string) get_user_meta( $userID, self::$userTokenKey, true );
	}
	 ...
	 * @return bool
	 */
	public static function revokeRefreshToken( int $userID = 0 ): bool {
		$userID = self::safeGetUserId( $userID );
		if ( empty( $userID ) ) {
			return false;
		}
